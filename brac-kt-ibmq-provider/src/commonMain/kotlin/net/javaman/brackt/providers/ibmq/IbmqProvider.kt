...
class IbmqProvider {
    ...
    companion object {
        ...
        @OptIn(ExperimentalTime::class)
        @JvmStatic
        private val JOB_POLL_INTERVAL = 2.seconds
     ...
     */
    suspend fun getJobsLimit() = ibmqApi.jobsLimit(accessToken, network.name, group.name, project.name, device.name)
     ...
     */
    suspend fun runExperiment(qc: QuantumCircuit, shots: Int = 1024, name: String = "") = runExperiment(
    ...
    suspend fun andWait(timeoutDuration: Duration = JOB_TIMEOUT_DURATION): ResultResponse {
        ...
        while (runResponse.status != JOB_STATUS_COMPLETE) {
