...
class IbmqApi {
    ...
    companion object {
        const val BASE_AUTH_URL = "https://auth.quantum-computing.ibm.com/api"
        const val BASE_API_URL = "https://api.quantum-computing.ibm.com/api"
 ...
 */
suspend fun IbmqApi.logInWithToken(request: LogInWithTokenRequest): LogInWithTokenResponse {
    logger.info { "Requesting access token with apiToken (${request.apiToken.censor()})" }
    val response = client.post<LogInWithTokenResponse>("${IbmqApi.BASE_AUTH_URL}/users/loginWithToken") {
 ...
 */
suspend fun IbmqApi.apiToken(accessToken: String, userId: String): ApiTokenResponse {
    logger.info { "Requesting API token with userId (${userId.censor()})" }
    val response = client.get<ApiTokenResponse>("${IbmqApi.BASE_AUTH_URL}/users/$userId/apiToken") {
        headers {
            append(IbmqApi.ACCESS_TOKEN_HEADER, accessToken)
        }
    }
 ...
 */
suspend fun IbmqApi.networks(accessToken: String): NetworksResponse {
    logger.info { "Requesting networks" }
    val response = client.get<NetworksResponse>("${IbmqApi.BASE_API_URL}/network") {
        headers.append(IbmqApi.ACCESS_TOKEN_HEADER, accessToken)
    }
 ...
 */
suspend fun IbmqApi.backends(accessToken: String): BackendsResponse {
    logger.info { "Requesting backends" }
    val response = client.get<BackendsResponse>("${IbmqApi.BASE_API_URL}/users/backends") {
        headers {
            append(IbmqApi.ACCESS_TOKEN_HEADER, accessToken)
        }
    }
 ...
 */
suspend fun IbmqApi.jobs(accessToken: String): JobsResponse {
    logger.info { "Requesting jobs" }
    val response = client.get<JobsResponse>("${IbmqApi.BASE_API_URL}/jobs/v2") {
        headers {
            append(IbmqApi.ACCESS_TOKEN_HEADER, accessToken)
        }
    }
...
suspend fun IbmqApi.jobsLimit(
    ...
    val response = client.get<JobsLimitResponse>(
        "${IbmqApi.BASE_API_URL}/network/$network/groups/$group" +
 ...
 */
suspend fun IbmqApi.runExperiment(
    accessToken: String,
 ...
 */
suspend fun IbmqApi.versions(accessToken: String, request: VersionsRequest, code: String): VersionsResponse {
    logger.info { "Attempting to update a version with codeId (${request.idCode})" }
    val response = client.post<VersionsResponse>("${IbmqApi.BASE_API_URL}/codes/$code/versions") {
        headers {
            append(IbmqApi.ACCESS_TOKEN_HEADER, accessToken)
        }
        contentType(ContentType.Application.Json)
 ...
 */
suspend fun IbmqApi.latest(accessToken: String, codeId: String): LatestResponse {
    logger.info { "Requesting latest version of code ($codeId)" }
    val response = client.get<LatestResponse>("${IbmqApi.BASE_API_URL}/codes/$codeId/versions/latest") {
        headers {
            append(IbmqApi.ACCESS_TOKEN_HEADER, accessToken)
        }
    }
 ...
 * Get all saved codes
 */
suspend fun IbmqApi.lastest(accessToken: String, userId: String): LastestResponse {
    logger.info { "Request lastest codes for userId (${userId.censor()})" }
    val response = client.get<LastestResponse>("${IbmqApi.BASE_API_URL}/users/$userId/codes/lastest") {
        headers {
            append(IbmqApi.ACCESS_TOKEN_HEADER, accessToken)
        }
    }
 ...
 * Upload a new code
 */
suspend fun IbmqApi.newCode(accessToken: String, request: NewRequest): NewResponse {
    logger.info { "Attempting to create new code" }
    val response = client.post<NewResponse>("${IbmqApi.BASE_API_URL}/codes") {
        headers {
            append(IbmqApi.ACCESS_TOKEN_HEADER, accessToken)
        }
        contentType(ContentType.Application.Json)
 ...
 */
suspend fun IbmqApi.job(accessToken: String, jobId: String): JobResponse {
    logger.info { "Requesting the status of job ($jobId)" }
    val response = client.get<JobResponse>("${IbmqApi.BASE_API_URL}/jobs/$jobId/v/1") {
        headers {
            append(IbmqApi.ACCESS_TOKEN_HEADER, accessToken)
        }
    }
 ...
 */
suspend fun IbmqApi.resultDownloadUrl(accessToken: String, jobId: String): ResultDownloadUrlResponse {
    logger.info { "Requesting result download URL for job ($jobId)" }
    val response = client.get<ResultDownloadUrlResponse>("${IbmqApi.BASE_API_URL}/jobs/$jobId/resultDownloadUrl") {
        headers {
            append(IbmqApi.ACCESS_TOKEN_HEADER, accessToken)
        }
    }
